...
switch($REQUEST_METHOD) {
	...
	$sql = "select str_title,num_mcode from TAB_MENU where num_oid = $_OID and str_type like 'board%' order by num_mcode asc";
	$bbs_list = $DB -> sqlFetchAll($sql);
	for($ii=0; $ii<count($bbs_list); $ii++) {
		$bbs_list[$ii][stitle] = $DB -> sqlFetchOne("select str_title from TAB_MENU where num_oid = $_OID and num_mcode = ".substr($bbs_list[$ii][num_mcode],0,2)." ");
	}
