...
$id = $serial;
			...
			if ($num_file) {
				$FILE_LIST_tmp = $FH->upload_process_tmp($_POST['timestamp'],$id);
...
$max = 0;
if($FILE_LIST_tmp[$max][str_ftype] == "gif" ||$FILE_LIST_tmp[$max][str_ftype] == "jpg" ||$FILE_LIST_tmp[$max][str_ftype] == "png" ) {

	$size=GetImageSize($FH->get_real_url($FILE_LIST_tmp[$max]['str_refile']));
}
?>
|||<?=$FILE_LIST_tmp[$max][str_ftype]?>|||<?=$FILE_LIST_tmp[$max][str_upfile]?>|||<?=$FILE_LIST_tmp[$max][num_serial]?>|||<?=$size[0]?>|||<?=$size[1]?>|||<?=$id?>|||<?=$FILE_LIST_tmp[$max][str_url]?>
