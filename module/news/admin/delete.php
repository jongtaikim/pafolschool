...
foreach($ids as $id) {
	if(!$id) continue;
	$sql = "SELECT STR_TEXT1,STR_TEXT2,STR_TEXT3,STR_THUMB FROM ".TAB_MAIN_BOARD." WHERE NUM_OID=$_OID AND STR_CODE='$code' AND NUM_SERIAL=$id";
	$data = $DB->sqlFetch($sql);
	
	$sql = "DELETE FROM ".TAB_MAIN_BOARD." WHERE NUM_OID=$_OID AND STR_CODE='$code' AND NUM_SERIAL=$id";
...
WebApp::redirect($URL->setVar(array('act'=>'.list','ids'=>'','code'=>$code)),'삭제되었습니다.');
?>