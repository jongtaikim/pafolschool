...
if ($pass) {
	...
	$data = $DB->sqlFetchArray("
				SELECT 
					str_pass,num_step,num_group,num_depth,str_text1,str_text2,str_text3,str_thumb,str_mov,num_unix_time
				FROM
					$ARTICLE_TABLE
				WHERE 
					num_oid=$oid AND
	...
	if ($DB->query($sql)) {
		...
		if($_SESSION['USERID']){
			...
			$sql = "select count(*) from $ARTICLE_TABLE where num_oid = $_OID and str_user = '".$_SESSION['USERID']."' and num_unix_time between $sdate and $edate";
	...
	$tmp_mov = explode(",",$data[str_mov]);
	

		for($ii=0; $ii<count($tmp_mov); $ii++) {

	//2008-11-05 종태 쓰레기 동영상 버리기			


	$fp1 = fopen("http://121.78.86.50/uadmin/content_delete_proc2.php?id=".$tmp_mov[$ii],"r");
	fclose($fp1);	
