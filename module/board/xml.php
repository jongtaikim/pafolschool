...
$total = $DB->sqlFetchOne("SELECT COUNT(*) FROM $ARTICLE_TABLE WHERE num_mcode=$mcode and str_rss = 'Y' and num_input_pass is null  $whereadd");
if(!$total) $total = 0;
$page = $_REQUEST['page'];
if (!$page) $page = 1;
...
$sql = "
	SELECT
			...
			str_title, str_email, num_hit, num_file, num_comment, num_input_pass,dt_date,str_category, str_thumb,num_rank,str_tmp1, str_tmp2, str_tmp3,  str_tmp4, str_tmp5, str_tmp6,  str_tmp7, str_tmp8, str_tmp9, str_tmp10
		FROM
			$ARTICLE_TABLE
		WHERE
			num_oid=$oid    AND num_mcode=$mcode   $whereadd order by num_group desc
