...
switch ($REQUEST_METHOD) {
	...
	$tpl->setLayout('admin');
	$tpl->define("CONTENT", Display::getTemplate("edit_image.htm"));
	...
	case "POST":
		...
		if($attachfile) {
			...
			$FTP->chmod("/hosts/".HOST."/board_tmp",707);
			$FTP->mkdir($file->Path);
			$FTP->chmod($file->Path,707);
			...
			if(!$file->FileSizeChk((3*1024*1024))){
				echo '<script>alert("파일용량이 너무 큽니다. 2MB 이상 이미지는 업로드 하실 수 없습니다.");   history.go(-1); </script>';
				exit;
			}
		...
		if($normal_gallery[0] > 500) {?>

			<!-- 2008-11-05 종태 이미지 리턴하기 -->
			<script type="text/javascript" language="javascript"> 
			parent.pasteHTMLDemo('<center><a href="<?=$filepath?><?=$file->SaveName?>"  rel="lightbox1"><img src="<?=$filepath?><?=$file->SaveName?>" width ="500" border="0" alt="<?=$alt?>" ></a>&nbsp;</center><br>', '<?=$ifrmObjId?>'); 
			parent.closewPop(2)
			</script> 
		...
		<? }else{ ?>
			...
			<!-- 2008-11-05 종태 이미지 리턴하기 -->
			<script type="text/javascript" language="javascript"> 
			parent.pasteHTMLDemo('<img src="<?=$filepath?><?=$file->SaveName?>" width ="<?=$normal_gallery[0]?>" border="0" alt="<?=$alt?>" >&nbsp;<br>', '<?=$ifrmObjId?>'); 
			parent.closewPop(2)
			</script> 
...
class FileUpload { 
	...
	function Resize_Image($convert_width="78",$convert_height="56",$resize_dir="",$sumnile) { 
			...
			//GDImageResize($dir."/".$file, $tdir."/".$file."_100", $img_w2, $img_h2);			
