...
$sql = "
	...
	FROM
		(SELECT
			...
			str_title, str_email, num_hit, num_file, num_comment, num_input_pass, TO_CHAR(dt_date,'YYYY-MM-DD') dt_date,str_category, str_thumb,num_rank, rownum AS rnum,str_tmp1, str_tmp2, str_tmp3,  str_tmp4, str_tmp5, str_tmp6,  str_tmp7, str_tmp8, str_tmp9, str_tmp10,str_nick,str_view
		FROM
			$ARTICLE_TABLE
		WHERE
			num_oid=$oid AND num_mcode=$mcode  AND  num_notice=0   $whereadd $ddrder
		)
	WHERE
...
SELECT
			/*+ INDEX_DESC ($ARTICLE_TABLE $ARTICLE_SEARCH_INDEX) */
			 num_mcode, num_serial, num_notice, num_group, num_step, num_depth, str_user, str_name, str_pass, str_hak, str_text1,
			str_title, str_email, num_hit, num_file, num_comment, num_input_pass, TO_CHAR(dt_date,'YYYY-MM-DD') dt_date, str_thumb, rownum AS rnum,str_tmp1, str_tmp2, str_tmp3,  str_tmp4, str_tmp5, str_tmp6,  str_tmp7, str_tmp8, str_tmp9, str_tmp10,str_nick,str_view
		FROM
			$ARTICLE_TABLE
		WHERE
...
SELECT
			/*+ INDEX_DESC ($ARTICLE_TABLE $ARTICLE_SEARCH_INDEX) */
			 num_mcode, num_serial, num_notice, num_group, num_step, num_depth, str_user, str_name, str_pass, str_hak, str_text1,
			str_title, str_email, num_hit, num_file, num_comment, num_input_pass, TO_CHAR(dt_date,'YYYY-MM-DD') dt_date,str_category, str_thumb, rownum AS rnum,str_tmp1, str_tmp2, str_tmp3,  str_tmp4, str_tmp5, str_tmp6,  str_tmp7, str_tmp8, str_tmp9, str_tmp10,str_nick,str_view
		FROM
			$ARTICLE_TABLE
		WHERE
...
$tpl->define("CONTENT2", WebApp::getTemplate("board/skin/".$skin."/list.htm"));
...
function cb_format_list(&$arr,$key,$param) {
	...
	if(!$titlelen)  $titlelen = 10;

	if(strtotime($arr['dt_date']) > '2009-03-03') $arr['str_nick'] = $arr['str_name'];
...
$arr['thumb_url'] = str_replace("%FILE_HOST%","isch_file.gnhost.kr/",$arr['thumb_url']);
...
$f_img =   $arr['thumb_url'];
$normal_gallery=GetImageSize("/www1/isch_file/hosts/"._OID."/menu/".$arr['str_thumb']); 
