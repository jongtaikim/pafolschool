...
for($ii=0; $ii<count($boad); $ii++) {
	
//////////////////////////////////////////////////////////////////////////////////////////////

$curl->url="http://www.ulleung.ms.kr/sub.asp?Menu_Code=".$bbs_id."&inMode=contents&idx=".$boad[$ii];
$res = $curl->curl_login();

$subject = explode('<td width="66%" align="left"><span class="bold">제목 : </span>',$res);
$subject = explode("</td>",$subject[1]);
$subject = $subject[0];	

if($subject){

$date = explode('<td width="30%" align="right" style="COLOR: #3D71A5;">',$res);
$date = explode(' ',$date[1]);
$date = $date[0];	

$name = explode('<td width="64%" align="left" style="FONT-WEIGHT: bolder;">',$res);
$name = explode('(',$name[1]);
$name = $name[0];	



$hit = explode('<td align="right" style="COLOR: #2F9558">Hit : ',$res);
$hit = explode('</td>',$hit[1]);
$hit = $hit[0];	

$content = explode('<td width="94%">',$res);
$content = explode('<td width="3%"></td>',$content[1]);
$content = $content[0];	
...
if($file2) {
 ...
 echo $subject." / ".$date." / ".$hit." / <TEXTAREA>".$content."</TEXTAREA><br>";
