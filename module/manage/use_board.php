...
$sql = "select count(*) from TAB_BOARD where num_oid = $oids and TO_CHAR(dt_date,'YYYY-MM-DD') >='$year_start' and TO_CHAR(dt_date,'YYYY-MM-DD')<='$year_last'";
 ...
 $row = array();
  for($i=0;$i<12;$i++)
  {
   ...
   $edate=date("Y-m-d",mktime(23,59,59,$i+1,$mmax[$i],$year));
	...
	$sql = "select count(*) from TAB_BOARD where num_oid = $oids and TO_CHAR(dt_date,'YYYY-MM-DD')>='$sdate' and TO_CHAR(dt_date,'YYYY-MM-DD')<='$edate'";
