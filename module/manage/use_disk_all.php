...
$sql = "select sum(num_size) from TAB_FILES where  TO_CHAR(dt_date,'YYYY-MM-DD') >='$year_start' and O_CHAR(dt_date,'YYYY-MM-DD')<='$year_last'";
 ...
 $row = array();
  for($i=0;$i<12;$i++)
  {
   ...
   $edate=date("Y-m-d",mktime(23,59,59,$i+1,$mmax[$i],$year));
	...
	$sql = "select sum(num_size) from TAB_FILES where  TO_CHAR(dt_date,'YYYY-MM-DD')>='$sdate' and TO_CHAR(dt_date,'YYYY-MM-DD')<='$edate'";


	$total_year_counter = $DB -> sqlFetchOne($sql);

if($total_year_counter) {
$row[$i][total_price]=$total_year_counter/(1024*1024) ;	
}
...
$row[$i][str_title]=$i+1;
 ...
 echo '<?xml version="1.0" encoding="euc-kr"?>';
	...
	$tpl->setLayout('blank');
	$tpl->define("CONTENT", Display::getTemplate("manage/use_disk.htm"));
