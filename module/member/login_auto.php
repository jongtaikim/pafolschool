...
$sql = "SELECT 
      ...
      if($data['num_auth']) {
		...
		if($_SESSION['USERID']){
		$sql = "select (num_login_point + num_board_point + num_commint_point + num_repaly_point) as point_total  from TAB_MEMBER WHERE num_oid=$_OID AND str_id='".$_SESSION['USERID']."'";
